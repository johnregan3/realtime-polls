...
class RT_Polls_Widget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			<script type="text/javascript">
				...
				$script_options = "{" . RT_POLLS::prep_options( $poll_id, 'widget' ) . "}";
				 ?>
				 jQuery(document).ready( function(){
					jQuery.plot( jQuery( "#widget-graph" ),  data_widget, <?php echo $script_options ?> );
				});
			</script>

			<div id="rt-polls-widget-legend">
				<?php
				$i = 1;
				foreach ( $labels_array as $label => $val ) : ?>
					<div class="rt-polls-widget-input">
						<input type="radio" value="<?php echo esc_attr( 'label-title-' . $i ) ?>" name="rt-polls-widget-checkbox" <?php if ( 1 == $i ) : echo "checked"; endif; ?> />
						<span class="checksquare" style="background-color: <?php echo esc_attr( $poll_fields['field-color-' . $i ] ); ?>;"></span>
						<span class="rt-polls-widget-label"><?php echo esc_html( $val ) ?></span>
					</div>
				<?php
				$i++;
				endforeach;
			...
			?>
			<div id="widget-message-area"></div>
		...
		function form($instance) {
			...
			<p>
				<select name="<?php echo $this->get_field_name( 'poll_id' ); ?>">
					<?php foreach ( $polls as $poll ) : ?>
						<option value="<?php echo $poll->ID ?>" <?php selected( $instance['poll_id'], $poll->ID ); ?>><?php echo esc_html( $poll->post_title ) ?></option>
					<?php endforeach; ?>
				</select>
			</p>
			<p>
