...
function gamify_stats_shortcode() {
	...
	if ( $user_log_array ) {
		...
		// Get array of all levels achieved
		$user_level_array = array();
		foreach ( $reverse_user_log_array as $timestamp => $value ) {
			if ( $value['activity_type'] == 'level' ) {
				$user_level_array[$timestamp] = $value;
			}
		}

		// Get array of all badges achieved
		$user_badges_array = array();
		foreach ( $reverse_user_log_array as $timestamp => $value ) {
			if ( $value['activity_type'] == 'badge' ) {
				$user_badges_array[$timestamp] = $value;
			}
		}

		// Get array of all badges achieved
		$user_adjustment_array = array();
		foreach ( $reverse_user_log_array as $timestamp => $value ) {
			if ( $value['activity_type'] == 'adjustment' ) {
				$user_adjustment_array[$timestamp] = $value;
			}
		}
	...
	if ( is_user_logged_in() ) : ?>
		...
		<div id="gamify-wp-user=stats-wrap">
			...
			<?php if ( $user_log_array ) : ?>
				...
				<?php $shortcode->produce_list( $user_level_array ); ?>
				<p class="gamify-badges-list"><?php _e( 'Recent Badges Earned', 'gamify' ) ?></p>
				<?php $shortcode->produce_list( $user_badges_array ); ?>
			<?php endif; ?>
		</div>
